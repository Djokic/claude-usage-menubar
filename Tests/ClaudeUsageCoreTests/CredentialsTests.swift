...
@Suite struct CredentialsTests {
    ...
    @Test func currentTokenReportsValidWhenNotExpired() async throws {
        ...
        let runner = FakeCommandRunner { _, _, _ in self.blob(access: "live", expiresAt: self.futureExpiry(fixedNow)) }
    ...
    @Test func currentTokenReportsExpiredButStillReturnsToken() async throws {
        let fixedNow = Date(timeIntervalSince1970: 1_000_000)
        let runner = FakeCommandRunner { _, _, _ in self.blob(access: "stale", expiresAt: self.pastExpiry(fixedNow)) }
        let store = CredentialStore(runner: runner, now: { fixedNow })
    ...
    @Test func neverWritesKeychain() async throws {
        let fixedNow = Date(timeIntervalSince1970: 1_000_000)
        let runner = FakeCommandRunner { _, _, _ in self.blob(access: "x", expiresAt: self.pastExpiry(fixedNow)) }
        let store = CredentialStore(runner: runner, now: { fixedNow })
